        ...
        | NESTED_SYM PATH_SYM TEXT_STRING_sys
          {
            ...
            Json_table_nested_path *np= new (thd->mem_root)
              Json_table_nested_path(lex->json_table_column_nest);
            np->set_path(thd, $3);
            *lex->json_table_column_nest->m_nested_hook= np;
            lex->json_table_column_nest->m_nested_hook= &np->m_next_nested;
            lex->json_table_column_nest= np;
          }
          json_table_columns_clause
          {
            LEX *lex=Lex;
            lex->json_table_column_nest= lex->json_table_column_nest->m_parent;
          }
        ;
...
table_function:
          JSON_TABLE_SYM '(' expr ',' TEXT_STRING_sys
          {
            ...
            if (unlikely(!jt || jt->m_nested_path.set_path(thd, $5)))
              MYSQL_YYABORT;
            Lex->json_table= jt;
            Lex->json_table_column_nest= &jt->m_nested_path;
          }
          json_table_columns_clause ')' opt_as ident_table_alias
          {
            ...
            if (!($$= Select->add_table_to_list(thd,
                           new (thd->mem_root) Table_ident(&$10), NULL,
